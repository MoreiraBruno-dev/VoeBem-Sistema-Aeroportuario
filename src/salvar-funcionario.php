		...
		case "cadastrar":
			...
			$sql = "INSERT INTO funcionario (nome_funcionario, email_funcionario, telefone_funcionario) VALUES ('{$nome}', '{$email}', '{$telefone}')";
   			...
   			if ($res == true) {
   				...
   				print "<script>location.href='?page=listar-funcionaro';</script>";
   			}
			break;
	case 'editar':
			$nome     = $_POST['nome_funcionario'];
            ...
            $sql = "UPDATE funcionario SET nome_funcionario='{$nome}',
            email_funcionario='{$email}',telefone_funcionario='{$telefone}' WHERE id_funcionario=".$_REQUEST['id_funcionario'];
